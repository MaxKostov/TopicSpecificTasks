...
public class CrazyLambdas {
    ...
    public static Predicate<String> lengthInRangePredicate(int min, int max) {
        return (s) -> {return ((min <= s.length()) && (s.length() < max));};
    }
    ...
    public static UnaryOperator<Function<String, String>> composeWithTrimFunction() {
        return (function) -> {return function.compose(String::trim);};
    }
    ...
    public static BiFunction<Map<String, IntUnaryOperator>, String, IntUnaryOperator> functionLoader() {
        return (map, name) -> {
            if (map.containsKey(name)) {
                return (value) -> {
                    return map.get(name).applyAsInt(value);
                };
            } else {
    ...
    public static <T, U extends Comparable<? super U>> Comparator<T> thenComparing(
        ...
        return (t1, t2) -> {
            int firstCompare = comparator.compare(t1, t2);
            if (firstCompare != 0) {
                return firstCompare;
            } else {
