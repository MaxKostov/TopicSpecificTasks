...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(account -> {
            return account.getBirthday().getMonth().equals(birthdayMonth);
        }).toList();
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(a -> {return a.getFirstName().length() + a.getLastName().length();}).sum();
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream().anyMatch(a -> {
            return a.getEmail().endsWith(emailDomain);
        });
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        Optional<Account> optionalAccount = accounts.stream().filter(account -> account.getEmail().equals(email)).findFirst();
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                ...
                .flatMap(firstName -> firstName.chars().mapToObj(c -> (char) c))
                .collect(Collectors.groupingBy(
                        c -> c,
                        Collectors.counting()
                ));
     ...
     * case, in all  that are equal or longer than nameLengthBound.
