...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            entities = entities.stream().filter(e -> e.getUuid() == null).toList();
        ...
        public static <T extends BaseEntity> boolean hasDuplicates (List<T> entities, T targetEntity) {
            long count = entities.stream().filter(e -> e.getUuid() == targetEntity.getUuid()).count();
            return count > 1L;
        }
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Iterable<T> entities) {
            return findMax(entities, CREATED_ON_COMPARATOR).orElseThrow(java.util.NoSuchElementException::new);
        }
